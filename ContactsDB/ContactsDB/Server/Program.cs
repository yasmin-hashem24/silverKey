...
app.MapPost("/login", async (HttpContext context, EdgeDBClient _edgeDbClient) =>
{
...
app.MapPost("/AddConactPost", async (HttpContext context, EdgeDBClient _edgeDbClient) =>
{
    var requestBody = await new StreamReader(context.Request.Body).ReadToEndAsync();
    var contact = JsonSerializer.Deserialize<InputContact>(requestBody);
